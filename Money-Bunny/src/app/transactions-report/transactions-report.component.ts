...
export class TransactionsReportComponent implements OnInit {
  ...
  constructor(private firestore: AngularFirestore, private auth: AuthenticationService, private router: Router) {
  ...
  removeRecurrency(transaction: any) {
    this.firestore.collection('transactions').valueChanges().subscribe((transactions: any) => {
      transactions.forEach((tr: any) => {
        if (tr['id'] == transaction['id']) {
          this.firestore.collection('transactions').doc(transaction['id']).update({recurrent_days: 0});
        }
      });
    });
